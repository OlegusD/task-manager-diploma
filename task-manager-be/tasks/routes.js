...
router.get('/', async (req, res) => {
    ...
    const { rows } = await query(
        `
    SELECT t.*, s.name AS status_name, p.name AS priority_name, u.name AS author_name
    FROM tasks t
...
router.post('/', async (req, res) => {
    const { title, description, status_id, priority_id, parent_id } = req.body || {}
    if (!title || !status_id || !priority_id)
        return res.status(400).json({ error: 'Missing fields' })

    const id = uuidv4()
    await query(
        `INSERT INTO tasks (id, title, description, status_id, priority_id, author_id, parent_id)
     VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [id, title, description ?? null, status_id, priority_id, req.user.id, parent_id ?? null]
    )

    await query(
        `INSERT INTO task_history (id, task_id, action, new_value, author_id)
     VALUES ($1,$2,'created',$3,$4)`,
        [uuidv4(), id, JSON.stringify({ title, status_id, priority_id, parent_id }), req.user.id]
    )

    res.status(201).json({ id })
})

// Получить одну задачу с комментариями/историей
...
router.patch('/:id', async (req, res) => {
    ...
    // Проверка автор/админ
    const { rows: owner } = await query(
        'SELECT author_id, status_id, priority_id, title, description, parent_id FROM tasks WHERE id=$1',
        [id]
    )
    ...
    const diffs = {}
    ;['title', 'description', 'status_id', 'priority_id', 'parent_id'].forEach((k) => {
